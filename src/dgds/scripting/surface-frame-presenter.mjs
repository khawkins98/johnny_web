...
export const presentSurfaceFrameOperation = (state, operation) => {
    switch (operation.type) {
        ...
        case FrameOperationType.DRAW_LINE:
            state.surface.drawLine(operation.x1, operation.y1, operation.x2, operation.y2, operation.color);
            break;

        case FrameOperationType.FILL_RECT:
            state.surface.fillRect(operation.x, operation.y, operation.width, operation.height, operation.color);
            break;

        case FrameOperationType.FILL_CIRCLE:
            state.surface.fillCircle(operation.x, operation.y, operation.radius, operation.color);
            break;
