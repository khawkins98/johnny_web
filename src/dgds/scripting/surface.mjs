...
const normalizeRect = (rect, width = SURFACE_WIDTH, height = SURFACE_HEIGHT) => rect
    ? { x: rect.x, y: rect.y, width: rect.width, height: rect.height }
    : fullSurface(width, height);
...
const intersectRect = (left, right) => {
    ...
    const rightEdge = Math.min(
        Math.floor(left.x + left.width),
        Math.floor(right.x + right.width),
    );
    const bottomEdge = Math.min(
        Math.floor(left.y + left.height),
        Math.floor(right.y + right.height),
    );
    return {
        x,
        y,
...
export const createSoftwareSurface = ({
    width = SURFACE_WIDTH,
    ...
    const blendChannels = (offset, red, green, blue, sourceAlpha) => {
        ...
        pixels[offset] = Math.round(((red * sourceAlpha)
            + Math.round(pixels[offset] * destinationAlpha * inverseAlpha / 255)) / outputAlpha);
        pixels[offset + 1] = Math.round(((green * sourceAlpha)
            + Math.round(pixels[offset + 1] * destinationAlpha * inverseAlpha / 255)) / outputAlpha);
        pixels[offset + 2] = Math.round(((blue * sourceAlpha)
            + Math.round(pixels[offset + 2] * destinationAlpha * inverseAlpha / 255)) / outputAlpha);
        pixels[offset + 3] = outputAlpha;
    };
    const blendPixel = (offset, rgba) => blendChannels(
        offset,
        rgba[0],
        rgba[1],
        rgba[2],
    ...
    const recalculateBounds = searchRect => {
        ...
        occupiedBounds = right < left ? null : {
            x: left,
            y: top,
            width: right - left + 1,
            height: bottom - top + 1,
    ...
    const surface = {
        ...
        drawSprite(image, x, y, { clip = extent, flipX = false } = {}) {
            ...
            let drawnRight = -1;
            let drawnBottom = -1;
            for (let destinationY = destination.y;
                destinationY < destination.y + destination.height;
                destinationY++) {
                const sourceY = destinationY - y;
                for (let destinationX = destination.x;
            ...
            if (drawnRight >= drawnLeft) occupiedBounds = mergeBounds(occupiedBounds, {
                x: drawnLeft,
                y: drawnTop,
        ...
        drawSurface(source, rect) {
            ...
            const sourceArea = source.bounds
                ? intersectRect(requested, source.bounds)
                : requested;
        ...
        replaceRegionFrom(source, rect) {
            ...
            for (let destinationY = area.y; destinationY < area.y + area.height; destinationY++) {
                for (let destinationX = area.x; destinationX < area.x + area.width; destinationX++) {
                    ...
                    if (destinationX < copyArea.x || destinationX >= copyArea.x + copyArea.width
                        || destinationY < copyArea.y || destinationY >= copyArea.y + copyArea.height) {
        ...
        fingerprint() {
            ...
            for (let y = area.y; y < area.y + area.height; y++) {
                for (let x = area.x; x < area.x + area.width; x++) {
                    ...
                    hash ^= pixels[offset]; hash = Math.imul(hash, 0x01000193);
                    hash ^= pixels[offset + 1]; hash = Math.imul(hash, 0x01000193);
                    hash ^= pixels[offset + 2]; hash = Math.imul(hash, 0x01000193);
                    hash ^= pixels[offset + 3]; hash = Math.imul(hash, 0x01000193);
