...
The developer menu separates **Start a debug run** controls from **Now playing** status. The ADS file, tag, and chapter fields describe only what the next button press will start; they do not mirror the event currently on screen. A **Playback mode** field makes the two behaviors explicit before the stable **Start Debug Run** action:
...
- **Complete chapter** discards the existing queue and asks the Johnny controller for a new compatible sequence anchored on that tag. An ordinary event becomes the first event; a final event becomes the planned finale. Walking, tide, island position, raft, holiday suppression, and the final wipe continue through the resulting sequence.
