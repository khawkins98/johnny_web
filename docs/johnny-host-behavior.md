...
Some ADS scenes start a looping child TTM with a negative run count. This is an authored lifetime in DGDS timer ticks, not a sprite-frame delay: for example, `VISITOR.ADS` starts the `MJCOCO.TTM` tree shake with `-180`, while that child alternates its own two frames every eight ticks. The runtime applies both clocks independently so the shake retains its authored cadence and still hands control back to the parent scene when its lifetime expires.
