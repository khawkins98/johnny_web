...
The campfire also exercises ADS's remove-before-add branch semantics. Its very-large-fire TTM completes one cycle, is moved behind the actors, and is stopped and re-added in the same logical branch so a fresh cycle remains visible while Johnny fetches and cooks the boot. The staged JavaScript collection must accept that replacement and clear the old instance's finished history; otherwise the fire vanishes during the walk even though the authored ADS does not stop it until the later hands-warmed branch.
