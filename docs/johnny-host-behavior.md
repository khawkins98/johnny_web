...
Walking is likewise host functionality. The scene catalogue supplies start/end spots and headings, and the browser joins them through the island bookmark graph. Sprite/frame/coordinate rows are decoded at runtime from offset `0x188ea` in the user's original `SCRANTIC.SCR`; `JOHNWALK.BMP` remains a normally decoded resource. No extracted walking table is checked in. The browser independently enumerates non-repeating routes through its recovered bookmark graph and uses injected randomness to select one. Johnny Reborn's path finder was consulted for the behavioral conclusion that the host selected among multiple viable routes, but its GPL implementation and transition matrix were not copied. Both projects' route choice remains an approximation of the lost executable algorithm.
...
Some ADS scenes start a child TTM with a negative run count. This is an authored lifetime in DGDS timer ticks, not a sprite-frame delay: for example, `VISITOR.ADS` starts the `MJCOCO.TTM` tree shake with `-180`, while that child alternates its own two frames every eight ticks. The same rule also repeats finite TTM bodies, such as the staged campfire sprites, until their cutoff; treating their first `END` as completion makes the fire stages and Johnny poses overlap incorrectly. The runtime applies the lifetime and frame clocks independently and hands control back to ADS only when the lifetime expires.

The campfire also exercises ADS's remove-before-add branch semantics. Its very-large-fire TTM completes one cycle, is moved behind the actors, and is stopped and re-added in the same logical branch. The original ADS host repeatedly evaluates the active segment and re-arms that zero-run-count sequence; Bottle marks this specific branch-rearmed replacement to restart until the authored `STOP_SCENE` after the hands-warmed action. Merely retaining its second completed frame is insufficient because later actor GET/PUT restores correctly retire completed overlapping layers, making the fire disappear throughout the boot routine.

The developer menu separates **Start a debug run** controls from **Now playing — host event** status. With **Follow host event** enabled (the default), the ADS file, tag, and chapter fields mirror the active host event. Interacting with a target selector pauses that synchronization so a different run can be configured without the current event overwriting it; re-enabling the checkbox snaps the controls back to live playback. A **Playback mode** field makes the two behaviors explicit before the stable **Start Debug Run** action:

- **Selected scene only** builds the selected tag with compatible island/story state, plays it once, and leaves the controller's existing queue untouched.
- **Complete chapter sequence** discards the existing queue and asks the Johnny controller for a new compatible sequence anchored on that tag. An ordinary event becomes the first event; a final event becomes the planned finale. Walking, tide, island position, raft, holiday suppression, and the final wipe continue through the resulting sequence.

The **Run plan** card states the resulting order in concrete terms before playback starts; the action button never changes its label or meaning based on scene metadata.

Starting either debug mode is an atomic host interruption. It cancels the currently captured walk, DGDS runtime, sound, and final wipe; clears transient foreground drawing; and then consumes the newest requested preview or controller plan. This matters because walking and wipes belong to the Johnny host rather than the DGDS runtime: stopping only ADS could otherwise allow an old walk or already-captured scene to appear after the new controls were selected. Repeated debug requests use latest-request-wins semantics.

The **Story Chapter** selector supplies one day of eligibility for unrestricted events; it is the original 11-day narrative counter, not an island-position control. It gates the Johnny/Mary/Suzy story events and advances the raft stage. A faithful sequence does not traverse several chapters because the original advanced this counter only when the system calendar changed. When an event fixes its historical chapter, the unavailable selector is hidden. Island position, tide, ocean, clouds, and day/night are separate sequence presentation choices. The contextual run-plan card explains whether the configured event starts the sequence, ends it after compatible lead-ins, or is a start-immediately one-event finale.

The status card shows queued/playing state, active and next host events, effective chapter, remaining events, tide, and planned finale. The controller publishes plan and current-event changes; the generic browser host adds the actually active selected-scene preview, and the UI also reconciles status every 250 ms in case a notification is missed at a runtime/interlude boundary. A selected-scene preview is labelled as such and names the queued chapter it will resume. This is intentionally a host-event view: the Enhanced-mode stage note may simultaneously name a nested TTM animation (for example, `COCONUT 360`) running inside the active ADS event. Those two labels describe different levels rather than conflicting selections. Automated tests anchor all 63 catalogue entries at both randomness boundaries and verify that every sequence terminates and retains compatible tide/position flags.
...
- [Johnny Reborn's path finder](https://github.com/jno6809/jc_reborn/blob/master/calcpath.c) was used to cross-check the behavioral finding that one route was chosen randomly from multiple candidates. Its implementation states that it is not the original executable's exact algorithm; this project independently derives candidates from its existing bookmark graph and retains that qualification.
